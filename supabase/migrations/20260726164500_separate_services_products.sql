-- Migration to separate Services and Products

-- 1. Add 'product' to catalog_item_type_enum
ALTER TYPE public.catalog_item_type_enum ADD VALUE IF NOT EXISTS 'product';

-- 2. Add future inventory fields to catalog_items
...
COMMENT ON COLUMN public.catalog_items.sku IS 'Stock Keeping Unit identifier.';
COMMENT ON COLUMN public.catalog_items.stock_quantity IS 'Current inventory level.';

-- 3. Add catalog_item_type to transaction_items to preserve historical type
ALTER TABLE public.transaction_items
  ADD COLUMN catalog_item_type public.catalog_item_type_enum NOT NULL DEFAULT 'service';

COMMENT ON COLUMN public.transaction_items.catalog_item_type IS 'Historical snapshot of the catalog item type.';

-- 4. Update complete_visit RPC to insert catalog_item_type
