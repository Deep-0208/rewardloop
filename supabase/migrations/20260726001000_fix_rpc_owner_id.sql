-- Fix RPC owner_id bug
-- The businesses table does not have an owner_id column, mapping is done via users table.

-- 1. Fix complete_visit
...
BEGIN
  ...
  PERFORM 1 FROM public.users WHERE business_id = p_business_id AND auth_user_id = auth.uid() AND role = 'owner';
...
-- 2. Fix create_customer_with_wallet
...
AS $$
  ...
  WHERE business_id = (SELECT business_id FROM public.users WHERE auth_user_id = auth.uid() AND role = 'owner' LIMIT 1)
