-- Migration: 20260802130000_fix_complete_visit_bigint.sql
-- Description: Fix complete_visit RPC return types and parameter types from INTEGER to BIGINT

DROP FUNCTION IF EXISTS public.complete_visit(uuid, uuid, jsonb, integer, text, uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.complete_visit(uuid, uuid, jsonb, bigint, text, uuid, uuid, uuid);

CREATE OR REPLACE FUNCTION public.complete_visit(
  p_idempotency_key UUID,
  p_customer_id UUID,
  p_items JSONB,
  p_reward_applied BIGINT,
...
BEGIN
  -- Security Hardening: Validate active business membership for p_created_by
  ...
  VALUES (
    p_business_id, p_created_by, 'TRANSACTION_CREATED'::public.audit_event_enum, 'transactions', v_transaction_id,
    jsonb_build_object('final_paid', v_final_paid, 'reward_used', p_reward_applied)
  );

  RETURN QUERY SELECT v_transaction_id, v_subtotal, p_reward_applied::BIGINT,
                      v_reward_earned, v_final_paid, v_new_wallet_balance, false;
END;
$$;

REVOKE ALL ON FUNCTION public.complete_visit(uuid, uuid, jsonb, bigint, text, uuid, uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.complete_visit(uuid, uuid, jsonb, bigint, text, uuid, uuid, uuid) TO authenticated, service_role;
