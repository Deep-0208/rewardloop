...
-- 1. Create generalized rate limits table for layered OTP rate limiting
CREATE TABLE IF NOT EXISTS public.rate_limits (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  target_type TEXT NOT NULL CHECK (target_type IN ('phone', 'business', 'ip')),
...
-- Enable RLS (though it's only manipulated by SECURITY DEFINER RPCs)
ALTER TABLE public.rate_limits ENABLE ROW LEVEL SECURITY;

-- Clean up the old table if no longer used
DROP TABLE IF EXISTS public.otp_rate_limits CASCADE;

-- Recreate the check_and_update_otp_cooldown RPC with the layered logic
DROP FUNCTION IF EXISTS public.check_and_update_otp_cooldown(TEXT, INTEGER, INTEGER, INTEGER);
...
AS $$
DECLARE
  v_phone_limit public.rate_limits%ROWTYPE;
  v_ip_limit public.rate_limits%ROWTYPE;
  v_business_limit public.rate_limits%ROWTYPE;
  v_now TIMESTAMPTZ := now();
BEGIN
  -- 1. Check Phone (5 per 10 mins)
  SELECT * INTO v_phone_limit FROM public.rate_limits WHERE target_type = 'phone' AND target_value = p_phone FOR UPDATE;
  IF FOUND THEN
    IF v_now - v_phone_limit.last_request_at < (p_cooldown_seconds * (2 ^ (v_phone_limit.request_count - 1))) * interval '1 second' THEN
      RETURN false;
    END IF;
    IF v_now - v_phone_limit.window_start_time < interval '10 minutes' AND v_phone_limit.request_count >= 5 THEN
      RETURN false;
    END IF;
  END IF;

  -- 2. Check IP (20 per 10 mins)
  IF p_ip IS NOT NULL AND p_ip != '' THEN
    SELECT * INTO v_ip_limit FROM public.rate_limits WHERE target_type = 'ip' AND target_value = p_ip FOR UPDATE;
    IF FOUND THEN
      IF v_now - v_ip_limit.window_start_time < interval '10 minutes' AND v_ip_limit.request_count >= 20 THEN
        RETURN false;
      END IF;
    END IF;
  END IF;

  -- 3. Check Business (100 per hour)
  IF p_business_id IS NOT NULL THEN
    SELECT * INTO v_business_limit FROM public.rate_limits WHERE target_type = 'business' AND target_value = p_business_id::TEXT FOR UPDATE;
    IF FOUND THEN
      IF v_now - v_business_limit.window_start_time < interval '1 hour' AND v_business_limit.request_count >= 100 THEN
        RETURN false;
      END IF;
    END IF;
  END IF;
  ...
  -- Updates/Inserts
  -- Phone
  IF v_phone_limit IS NOT NULL THEN
    UPDATE public.rate_limits SET
      last_request_at = v_now,
      window_start_time = CASE WHEN v_now - window_start_time >= interval '10 minutes' THEN v_now ELSE window_start_time END,
      request_count = CASE WHEN v_now - window_start_time >= interval '10 minutes' THEN 1 ELSE request_count + 1 END
    WHERE id = v_phone_limit.id;
  ELSE
    INSERT INTO public.rate_limits (target_type, target_value) VALUES ('phone', p_phone);
  END IF;

  -- IP
  IF p_ip IS NOT NULL AND p_ip != '' THEN
    IF v_ip_limit IS NOT NULL THEN
      UPDATE public.rate_limits SET
        last_request_at = v_now,
        window_start_time = CASE WHEN v_now - window_start_time >= interval '10 minutes' THEN v_now ELSE window_start_time END,
        request_count = CASE WHEN v_now - window_start_time >= interval '10 minutes' THEN 1 ELSE request_count + 1 END
    ...
    ELSE
      INSERT INTO public.rate_limits (target_type, target_value) VALUES ('ip', p_ip);
    END IF;
  END IF;

  -- Business
  IF p_business_id IS NOT NULL THEN
    IF v_business_limit IS NOT NULL THEN
      UPDATE public.rate_limits SET
        last_request_at = v_now,
        window_start_time = CASE WHEN v_now - window_start_time >= interval '1 hour' THEN v_now ELSE window_start_time END,
        request_count = CASE WHEN v_now - window_start_time >= interval '1 hour' THEN 1 ELSE request_count + 1 END
      WHERE id = v_business_limit.id;
    ELSE
      INSERT INTO public.rate_limits (target_type, target_value) VALUES ('business', p_business_id::TEXT);
    END IF;
  END IF;
...
REVOKE ALL ON FUNCTION public.check_and_update_otp_cooldown(TEXT, TEXT, UUID, INTEGER) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.check_and_update_otp_cooldown(TEXT, TEXT, UUID, INTEGER) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.check_and_update_otp_cooldown(TEXT, TEXT, UUID, INTEGER) TO service_role;

-- 2. Audit and Harden RPCs (auth.uid, business_id, ownership, search_path)

-- Re-create purge_expired_otp_rate_limits to use the new rate_limits table
...
-- Complete Visit Hardening
-- Previously created in 20260724120000_complete_visit_transaction.sql
-- We need to add ownership validation to complete_visit.
-- I will redefine it but I need to see its current definition.
...
BEGIN
  ...
  PERFORM 1 FROM public.businesses WHERE id = p_business_id AND owner_id = auth.uid();
  IF NOT FOUND THEN RAISE EXCEPTION 'Not authorized for this business' USING ERRCODE = '42501'; END IF;
...
REVOKE ALL ON FUNCTION public.create_customer_with_wallet(TEXT, TEXT, UUID, UUID) FROM PUBLIC;
