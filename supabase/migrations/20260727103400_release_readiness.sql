-- Migration for RR-03 (Rounding Fix) and RR-06 (OTP Reward Binding)

-- 1. Add reward_amount_paise to otp_requests to bind an OTP to a specific redemption amount
ALTER TABLE public.otp_requests 
ADD COLUMN IF NOT EXISTS reward_amount_paise INTEGER CHECK (reward_amount_paise >= 0);

COMMENT ON COLUMN public.otp_requests.reward_amount_paise IS 'The specific reward amount (in paise) this OTP is authorizing. Used to prevent redemption manipulation after OTP confirmation.';

-- 2. Update complete_visit RPC to fix rounding and validate OTP reward amount
...
BEGIN
  ...
  -- RR-03 Fix: Use floor((val + 50)/100) to match TypeScript's Math.floor((val + 50)/100) half-up rounding
  ...
  IF p_reward_applied > 0 THEN
    ...
    -- RR-06 Fix: Validate the OTP was issued for this specific reward amount
